...
# free local compute — corpus refresh + re-baseline + board (no model spend)
python3 historical.py --leagues KXNBAGAME 2>&1 | tail -2 || true
python3 analyze.py 2>&1 | tail -2 || true
python3 boards.py 2>&1 | tail -1 || true

# ONE creative iteration via your Max-authed Claude (headless, restricted tools, paper-only)
